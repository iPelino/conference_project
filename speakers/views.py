...
speakers = [
        ...
        {
            "id":  labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. "
        },
        {
            "id": 3,
            "name": "Alice Uwera",
...
def speaker_view(request, id):
    ...
    for spek in speakers:
        if spek["id"] == int(id):
            speaker = spek
    return render(request, "speakers/speaker.html", {"speaker": speaker })
...
def update_speaker_view(request, id):
    ...
    for spek in speakers:
        if spek["id"] == int(id):
            speaker = spek
    return render(request, "speakers/updatedspeaker.html", {"speaker": speaker})
